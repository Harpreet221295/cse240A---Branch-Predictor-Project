...
main(int argc, char *argv[])
{
  ...
  double running_mean_prediction_time = 0;
  //long long count = 0
  ...
  while (read_branch(&pc, &outcome)) {
    ...
    double new_val = end_time - start_time;

    
    running_mean_prediction_time = running_mean_prediction_time + new_val;
  ...
  printf("Mean prediction time: %lf\n", running_mean_prediction_time / num_branches);
  // Cleanup
  fclose(stream);
  free(buf);
